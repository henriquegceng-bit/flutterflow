...
class MeuCarrinhoSupabaseUser extends BaseAuthUser {
  MeuCarrinhoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> meuCarrinhoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MeuCarrinhoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
